...
setwd("D:/CINTHIA/DIAO/ProyectoCovid/09")
...
muestra_ponde %>% 
  summarise(Total = sum(fe_mzn), 
            pob_total = sum(fe_mzn*POB20),
            viviendas = sum(fe_mzn * VIV2))

# valores reales
marco %>% 
  summarise(viviendas = sum(VIV2),
            pob_total = sum(POB20, na.rm = TRUE))
